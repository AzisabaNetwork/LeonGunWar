...
public class KDTeamDistributor implements TeamDistributor {
   ...
   * プレイヤーがAceか否かを判定します。
   *
   * <p>Ace条件: 月のキル数が3000以上 or KD1.2以上
   *
   * <p>例外: 累計キル数が1000未満の人は除外
   */
   ...
   *
   * <p>計算式: KDx1000 + 一か月のキル数÷10
   *
   * <p>例外: 累計キル数が100未満の人は上記の「KDx1000」 を800に固定する
   */
  ...
  public void distributePlayer(Player player, List<Team> teams) {
    ...
    if (isACE(player)) {
      ...
      teams.stream()
          .min(
              Comparator.comparing(manager::getTeamAcePowerLevel)
                  .thenComparing(manager::getTeamPowerLevel)
                  .thenComparing(Team::getSize)
                  .thenComparing(manager::getCurrentTeamPoint))
    ...
    teams.stream()
        .min(
            Comparator.comparing(manager::getTeamPowerLevel)
                .thenComparing(Team::getSize)
                .thenComparing(manager::getCurrentTeamPoint))
        .ifPresent(lowTeam -> lowTeam.addEntry(player.getName()));
  }
