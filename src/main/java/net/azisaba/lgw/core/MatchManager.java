...
public class MatchManager {
  ...
  public void startMatch() {
    ...
    Bukkit.broadcastMessage(
        Chat.f(
            "{0}&7制限時間 &c{1}",
            LeonGunWar.GAME_PREFIX, SecondOfDay.f(matchMode.getDuration().getSeconds())));
    // 勝利条件を発表
    Bukkit.broadcastMessage(
  ...
  public void finalizeMatch() {
    ...
    // Entry削除
    teams.values().forEach(team -> new ArrayList<>(team.getEntries()).forEach(team::removeEntry));
  ...
  /**
   * ゲームの残り時間を操作するタイマータスクを起動します 基本はMatchTimeChangedEventを利用して、イベントからゲームを操作するため このタスクでは基本他の動作を行いません
   */
  ...
  public boolean addPlayerIntoBattle(Player p) {
    ...
    // チームを取得する
    Map.Entry<BattleTeam, Team> entry =
    ...
    if (leader != null) {
      p.sendMessage(
          Chat.f(
              "{0}&7所属チームのリーダーは &r{1} &7です！", LeonGunWar.GAME_PREFIX, leader.getPlayerListName()));
    }
    ...
    boolean enableEntry =
        data.isSet("LeonGunWar.EntryOnRejoin") && data.getBoolean("LeonGunWar.EntryOnRejoin");
  ...
  public void setLeaderAtRandom(BattleTeam team) {
    ...
    Bukkit.broadcastMessage(
        Chat.f("{0}{1} &7のリーダーが新しいプレイヤーに更新されました！", LeonGunWar.GAME_PREFIX, team.getTeamName()));
    ...
    plist.forEach(
        p ->
            p.sendMessage(
                Chat.f(
                    "{0}&7チームのリーダーに &r{1} &7が選ばれました！",
                    LeonGunWar.GAME_PREFIX, target.getPlayerListName())));
  ...
  protected void onDisablePlugin() {
    ...
    plist.forEach(
        p -> {
          ...
          p.sendMessage(
              Chat.f(
                  "&7[Your Score] {0} {1} Kill(s), {2} Death(s), {3} Assist(s)",
                  p.getName(), kills, deaths, assists));
  ...
  private void setUpPlayer(Player p, BattleTeam team) {
    ...
    p.getActivePotionEffects()
        .forEach(potionEffect -> p.removePotionEffect(potionEffect.getType()));
    // 防具を装備
   ...
   *
   * <p>team1からteam2が±1500ならtrue それ以外ならfalse
   *
   * @param team1,team2 チームのパワーレベル
