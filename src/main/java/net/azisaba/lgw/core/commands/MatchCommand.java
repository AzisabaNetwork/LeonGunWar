...
public class MatchCommand implements CommandExecutor, TabCompleter {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if (sender instanceof Player
        && cooldown.getOrDefault(((Player) sender).getUniqueId(), 0L) + 1000
    ...
    if (Args.check(args, 0, "entry")) {
      ...
      if (success) { // エントリーした場合
        ...
        if (!self) {
          sender.sendMessage(Chat.f("{0}&r{1} &7をエントリーさせました。", prefix, target.getDisplayName()));
        }
      } else { // すでにエントリーしている場合
        if (self) {
          sender.sendMessage(Chat.f("{0}&cあなたは既に参加しています。", prefix));
        } else {
          sender.sendMessage(Chat.f("{0}&r{1} &cは既に参加しています。", prefix, target.getDisplayName()));
        }
      }
    ...
    if (Args.check(args, 0, "leave")) {
      ...
      if (success) { // エントリー解除した場合
        ...
        if (!self) {
          sender.sendMessage(Chat.f("{0}&r{1} &7をゲームから退出させました。", prefix, target.getDisplayName()));
        }
      } else { // エントリーしていない場合
        if (self) {
          sender.sendMessage(Chat.f("{0}&7あなたはエントリーしていません。", prefix));
        } else {
          sender.sendMessage(Chat.f("{0}&r{1} &cは、エントリーしていません。", prefix, target.getDisplayName()));
        }
      }
    ...
    if (Args.check(args, 0, "rejoin")) {
      ...
      if (success) { // 途中参加した場合
        ...
        if (!self) {
          sender.sendMessage(Chat.f("{0}&r{1} &7を途中参加させました。", prefix, target.getDisplayName()));
        }
      } else { // すでに試合に参加している場合
        if (self) {
          sender.sendMessage(Chat.f("{0}&cあなたはすでに試合に参加しています。", prefix));
        } else {
          sender.sendMessage(Chat.f("{0}&r{1} &cはすでに試合に参加しています。", prefix, target.getDisplayName()));
        }
      }
      return true;
    }
