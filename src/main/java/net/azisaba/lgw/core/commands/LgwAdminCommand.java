...
public class LgwAdminCommand implements CommandExecutor, TabCompleter {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if (Args.check(args, 0, "teleport", "tp")) {
      ...
      List<GameMap> correctMapList =
          LeonGunWar.getPlugin().getMapsConfig().getAllGameMap().stream()
              .filter(
                  map ->
                      map.getMapName().equalsIgnoreCase(mapName)
    ...
    if (Args.check(args, 0, "showdata")) {
      ...
      sender.sendMessage(
          Chat.f("{0}&cMatch Data: {1}", LeonGunWar.GAME_PREFIX, mode.getModeName()));
      return true;
    }
