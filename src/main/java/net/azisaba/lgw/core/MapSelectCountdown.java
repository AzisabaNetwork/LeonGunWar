...
public class MapSelectCountdown {
  ...
  public void startCountdown(Set<GameMap> maps, MatchMode mode) {
    ...
    Optional.ofNullable(task.get())
        .ifPresent((task) -> task.runTaskTimer(LeonGunWar.getPlugin(), 0, 20));
  }
  ...
  public int getVote(int index) {
    ...
    return Optional.ofNullable(task.get()).map(task -> task.getVoteFor(index)).orElse(-1);
  }
  ...
  public int getTimeLeft() {
    ...
    return Optional.ofNullable(task.get()).map(task -> task.getTimeLeft().get()).orElse(-1);
  }
