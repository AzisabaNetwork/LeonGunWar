...
public class MapSelectCountdownTask extends BukkitRunnable {
  ...
  @Getter private final List<GameMap> maps;
  @Getter private final MatchMode mode;

  private final HashMap<Integer, AtomicInteger> votes = new HashMap<>();

  @Getter private final AtomicInteger timeLeft = new AtomicInteger(20);
  ...
  public void run() {
    ...
    if (timeLeft.get() <= 0) {
      ...
      String distributorName =
          LeonGunWar.getPlugin().getManager().getTeamDistributor().getDistributorName();
      String mapName =
          Optional.ofNullable(LeonGunWar.getPlugin().getManager().getCurrentGameMap())
      ...
      Bukkit.getOnlinePlayers()
          .forEach(player -> player.playSound(player.getLocation(), Sound.BLOCK_NOTE_PLING, 1, 1));
    ...
    if (timeLeft.get() <= 5) {
      String msg =
          Chat.f("{0}&7Map決定まで残り &c{1}&7", LeonGunWar.GAME_PREFIX, SecondOfDay.f(timeLeft.get()));
