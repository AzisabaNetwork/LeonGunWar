...
public class LeonGunWar extends JavaPlugin {
  ...
  // plugin
  @Getter private static LeonGunWar plugin;

  @Getter private static JSONMessage quickBar;
  ...
  public void onEnable() {
    plugin = this;
    quickBar =
        JSONMessage.create(Chat.f("&7[&bQuick&7] ここをクリック → "))
            .then(Chat.f("&a[エントリー]"))
    ...
    Bukkit.getPluginManager()
        .registerEvents(
            new AdminChatListener(
                (AdminChatCommand) Bukkit.getPluginCommand("adminchat").getExecutor()),
            this);
