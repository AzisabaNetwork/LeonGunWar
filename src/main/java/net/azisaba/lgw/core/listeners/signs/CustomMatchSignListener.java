...
/**
 * 次に実行するカスタム試合のあれこれを指定する看板&GUI ACTIVEとINACTIVEを切り替えるときはスニークをしながら右クリックで可能 破壊するときはスニークしながら左クリックで可能
 *
 * @author Mr_IK Thanks: siloneco
 */
public class CustomMatchSignListener implements Listener {

  private final ItemStack no_limit,
      matchpoint,
      main_limit,
      sub_limit,
      granade_limit,
  ...
  public void onInventoryClick(InventoryClickEvent e) {
    ...
    if (mode == null) {
      Bukkit.getLogger()
          .info(openingInv.getTitle().substring(openingInv.getTitle().indexOf(Chat.f("&e")) + 2));
      return;
    }
    ...
    if (distributor != null) {
      ...
      Bukkit.broadcastMessage(
          Chat.f("{0}&7勝利条件  {1}", LeonGunWar.GAME_PREFIX, CustomTDMListener.getWinCase()));
      Bukkit.broadcastMessage(
      ...
      if (e.getSlot() == 0) {
        ...
        if (Chat.r(itemname).equalsIgnoreCase("マッチ終了ポイント : 50P")) {
          e.getClickedInventory()
              .setItem(2, create(Material.EMERALD, Chat.f("&eマッチ終了ポイント : &a100P")));
        } else if (Chat.r(itemname).equalsIgnoreCase("マッチ終了ポイント : 100P")) {
        ...
        if (Chat.r(itemname).equalsIgnoreCase("メイン武器射撃 : 可能")) {
          e.getClickedInventory()
              .setItem(4, create(Material.SUGAR_CANE, Chat.f("&eメイン武器射撃 : &c不可能")));
        } else if (Chat.r(itemname).equalsIgnoreCase("メイン武器射撃 : 不可能")) {
        ...
        if (Chat.r(itemname).equalsIgnoreCase("グレネード投擲 : 可能")) {
          e.getClickedInventory()
              .setItem(8, create(Material.SLIME_BALL, Chat.f("&eグレネード投擲 : &c不可能")));
        } else if (Chat.r(itemname).equalsIgnoreCase("グレネード投擲 : 不可能")) {
