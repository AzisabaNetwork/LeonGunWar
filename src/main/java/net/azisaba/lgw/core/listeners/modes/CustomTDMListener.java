...
public class CustomTDMListener implements Listener {
  ...
  public static final HashMap<String, Integer> customLimit = new HashMap<>();

  // CrackShotAPI
  private static final CSDirector director = JavaPlugin.getPlugin(CSDirector.class);

  // customLimitで開発者が役に立つString
  public static final String MAIN_WEAPON = "primary_weapons";
  public static final String SUB_WEAPON = "sub_weapons";
  public static final String GRENADE = "grenade_weapons";
    ...
    if (e.getCurrentPoint() >= matchpoint) {
      ...
      MatchFinishedEvent event =
          new MatchFinishedEvent(
              manager.getCurrentGameMap(),
              Collections.singletonList(e.getTeam()),
              manager.getTeamPlayers());
      Bukkit.getPluginManager().callEvent(event);

      // メッセージ表示
    } else if (e.getCurrentPoint() % 50 == 0
        || matchpoint - e.getCurrentPoint() == 10
        || matchpoint - e.getCurrentPoint() == 5) {
      // 50区切り/残り10/残り5ならメッセージを表示
      Bukkit.broadcastMessage(
          Chat.f(
              "{0}&7残り &e{1}キル &7で &r{2} &7が勝利！",
              LeonGunWar.GAME_PREFIX, matchpoint - e.getCurrentPoint(), e.getTeam().getTeamName()));
    }
  }
    ...
    for (String invGroup : groupList) {
      ...
      for (int i = 0; i < 9; ++i) {
        ...
        if (checkItem != null && director.itemIsSafe(checkItem)) {
          ...
          if (checkParent != null) {
            String groupCheck =
                director.getString(checkParent[0] + ".Item_Information.Inventory_Control");
