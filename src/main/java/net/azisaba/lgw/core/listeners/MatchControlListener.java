...
public class MatchControlListener implements Listener {
  ...
  public void matchFinishDetector(MatchTimeChangedEvent e) {
    ...
    List<BattleTeam> winners =
        Arrays.stream(BattleTeam.values())
            .filter(
                team -> LeonGunWar.getPlugin().getManager().getCurrentTeamPoint(team) == maxPoint)
  ...
  public void onMatchFinished(MatchFinishedEvent e) {
    ...
    // MVPのプレイヤーを取得
    List<KDPlayerData> mvpPlayers =
        LeonGunWar.getPlugin().getManager().getKillDeathCounter().getMVPPlayer();
    // MVPプレイヤーのメッセージ
    List<String> resultMessages =
        new ArrayList<>(Collections.singletonList(Chat.f("&d=== Team Point Information ===")));
    ...
    if (!mvpPlayers.isEmpty()) {
      for (KDPlayerData data : mvpPlayers) {
        resultMessages.add(
            Chat.f(
                "&c[MVP] {0} {1} Kill(s), {2} Death(s), {3} Assist(s)",
                data.getPlayerName(), data.getKills(), data.getDeaths(), data.getAssists()));
      }
    }
          ...
          .forEach(
              wonTeam -> {
                ...
                Bukkit.broadcastMessage(
                    Chat.f(
                        "{0}{1} &7が &6勝利 &7しました！", LeonGunWar.GAME_PREFIX, wonTeam.getTeamName()));
              });
    }
  }
  ...
  public void actionbarUpdater(MatchTimeChangedEvent e) {
    ...
    for (Player p : players) {
      ...
      if (actionBar == null) {
        actionBar =
            LeonGunWar.getPlugin().getManager().getKillDeathCounter().getDefaultActionBar(p);
      }
  ...
  public void onPlayerKickedMatch(PlayerKickMatchEvent e) {
    ...
    for (List<Player> playerList : playerMap.values()) {
      if (playerList.size() <= 0) {
        ...
        // イベント作成
        MatchFinishedEvent event =
            new MatchFinishedEvent(manager.getCurrentGameMap(), new ArrayList<>(), playerMap);
        // 呼び出し
        Bukkit.getPluginManager().callEvent(event);
        break;
      }
